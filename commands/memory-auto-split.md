...
## PROSE strategy direction — verified, now derived per file (resolved)

An earlier draft of this command hardcoded "head = oldest, tail = newest" (content appended at the end) as an unverified assumption, flagged back as an open question. **Verified against the real motivating file: `pending.md` is newest-first — new items are PREPENDED at the top, not appended at the bottom.** Hardcoding the *other* direction instead would have had the exact same fragility, just backwards, so `hooks/auto-split.sh` now **derives** the direction per file rather than assuming either one:
...
- **Top date newer than bottom date → newest-first:** the oldest content is the suffix (bottom) — shard the suffix, keep the prefix (top) live.
- **Top date older than bottom date → newest-last:** the oldest content is the prefix (top) — shard the prefix, keep the suffix (bottom) live. (The direction the earlier draft assumed unconditionally — still correctly handled, just no longer hardcoded.)
- **No datestamp anywhere, or only one distinct date (no direction to derive) → fails closed**, reported plainly, rather than guessing. In practice this means a genuinely dateless `charter.md` (the common case — goals/requirements prose rarely carries dates) is refused and left for manual trimming; `pending.md`, which does carry dates in practice, is not affected.
- Either direction, the fence-safety nudge always moves toward the SHARD side (never the KEPT side) — the ceiling-safe direction, whichever end that happens to be.
- The script's `SCRIPT_OK` output for the PROSE strategy now includes `PROSE_DIRECTION=<newest-first|newest-last>` for operator visibility — worth a glance if it's ever surprising.

Both directions and both fail-closed paths (dateless, single-date) are covered in `tests/run-autosplit.sh`.
