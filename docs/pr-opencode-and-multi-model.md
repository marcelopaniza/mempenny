...
**Author context:** Research performed from an opencode session running GLM, against a Claude Code-populated memory dir at `~/.claude/projects/-mnt-data-boardero/memory/`.
...
**Fix:** **Keep the slug rule as-is** — `sed 's|/|-|g; s|^-||'`. From opencode's cwd `/mnt/data/boardero`, the rule produces `-mnt-data-boardero`, and `~/.claude/projects/-mnt-data-boardero/memory/` exists because Claude Code sessions in the same project put it there. **This is the single biggest unblock:** opencode becomes a read/clean/restore client on Claude-authored memory, with zero setup. Already verified empirically — the dir exists.
